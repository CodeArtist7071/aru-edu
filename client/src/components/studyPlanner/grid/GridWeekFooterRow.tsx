...
export const GridWeekFooterRow: React.FC<GridWeekFooterRowProps> = ({
  ...
  return (
    <tr className="bg-slate-50 border-t-2 border-slate-200 shadow-inner">
      ...
      {/* Week 1 Donut (7 Days) */}
      <td colSpan={7} className="border-r border-slate-100 p-0 text-center" style={{ backgroundColor: `${WEEK_COLORS[0]}90` }}>
        <GridWeekDonut percent={weeklyDone[0] || 0} color={`${WEEK_COLORS[0]}`} label="Week 1" />
      </td>
      {/* Week 2 Donut (7 Days) */}
      <td colSpan={7} className="border-r border-slate-100 p-0 text-center" style={{ backgroundColor: `${WEEK_COLORS[1]}90` }}>
        <GridWeekDonut percent={weeklyDone[1] || 0} color={`${WEEK_COLORS[1]}`} label="Week 2" />
      </td>
      {/* Week 3 Donut (7 Days) */}
      <td colSpan={7} className="border-r border-slate-100 p-0 text-center" style={{ backgroundColor: `${WEEK_COLORS[2]}90` }}>
        <GridWeekDonut percent={weeklyDone[2] || 0} color={`${WEEK_COLORS[2]}`} label="Week 3" />
      </td>
      {/* Week 4 Donut (7 Days) */}
      <td colSpan={7} className="border-r border-slate-100 p-0 text-center" style={{ backgroundColor: `${WEEK_COLORS[3]}90` }}>
        <GridWeekDonut percent={weeklyDone[3] || 0} color={`${WEEK_COLORS[3]}`} label="Week 4" />
      </td>
      {/* Week 5 / Extra Days (Remaining Days) */}
      <td colSpan={days.length - 28} className="p-0 text-center" style={{ backgroundColor: `${WEEK_COLORS[4]}90` }}>
        {days.length > 28 && (
