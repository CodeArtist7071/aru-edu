...
export const IOSInstallPrompt = () => {
  ...
  return (
    <div className="fixed bottom-[calc(6.5rem+env(safe-area-inset-bottom))] left-4 right-4 z-999 animate-buttery-slide">
      <div className="rounded-2xl bg-primary border border-primary shadow-ambient-lg p-5 relative overflow-hidden group">
        {/* <div className="absolute inset-0 bg-primary opacity-100 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" /> */}
        
        {/* Close Button */}
        <button 
          onClick={handleDismiss}
          className="absolute top-3 right-3 p-1.5 text-white hover:text-primary hover:bg-primary rounded-full transition-all"
          aria-label="Close"
        >
          <X size={18} />
        </button>
        ...
        <div className="flex items-start gap-4">
          <div className="bg-on-surface p-3 rounded-xl text-primary shrink-0">
            <PlusSquare size={24} />
          </div>
          <div className="flex-1">
            <h3 className="text-sm font-semibold text-white flex items-center gap-1.5">
              Install Arumind
              <span className="text-[10px] bg-primary/10 text-white px-1.5 py-0.5 rounded-full font-medium">Recommended</span>
            </h3>
            ...
            <div className="mt-4 flex flex-col gap-2.5">
              <div className="flex items-center gap-3 text-xs text-on-surface-variant bg-surface-container-low p-2.5 rounded-xl border border-outline-variant">
